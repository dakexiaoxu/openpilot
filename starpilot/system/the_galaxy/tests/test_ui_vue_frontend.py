...
def test_ui_uses_same_backend_endpoints():
  ...
  assert '/assets/components/tools/device_settings_layout.json?v=settings-zh-5' in api
...
def test_ui_language_selector_uses_shared_device_language_setting():
  ...
  assert 'SUPPORTED_CODES.has(code) ? code : DEFAULT_LANGUAGE' in i18n
  assert "LanguageSetting || \"en\"" not in settings
